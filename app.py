...
def show_login_page():
    ...
    st.title("🔐 NovaCart BI Assistant")
    st.caption("Employee Login")
    ...
    with st.form("login_form"):
        ...
        if submitted:
            ...
            if employee:
                ...
                if conversations:
                    first_conversation_id = str(conversations[0]["conversation_id"])
                    load_conversation(first_conversation_id)
                    st.query_params["conversation_id"] = first_conversation_id
                else:
